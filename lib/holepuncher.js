...
function holepunch (socket, addr, lowTTL) {
  console.log('sending holepunch msg to', addr, lowTTL)
...
function matchAddress (localAddresses, remoteLocalAddresses) {
  localAddresses.sort()
  remoteLocalAddresses.sort((a, b) => a.host > b.host ? 1 : -1)
  localAddresses.sort((a, b) => a.host > b.host ? 1 : -1)

  console.log('matchAddress matching local:', localAddresses, '\nto remote', remoteLocalAddresses)
  const nonDockerRemoteLocalAddresses = [...remoteLocalAddresses].filter(a => !a.host.startsWith('172.17')) // Filter out probably docker addresses
  console.log('filtered non docker remoteLocals:', nonDockerRemoteLocalAddresses)
  const nonDockerLocals = [...localAddresses].filter(a => !a.host.startsWith('172.17'))
  console.log('filtered non docker locals', nonDockerLocals)
  // if (nonDockerLocals.length > 0) { // In case it noticed a local network possibility (many false positives though...)
  //   return nonDockerLocals[0]
  // }
  ...
  for (const localAddress of localAddresses) {
    console.log('testing localAddress', localAddress)
    // => 192.168.122.238
    const a = localAddress.host.split('.')

    for (const remoteAddress of remoteLocalAddresses) {
      console.log('testing remoteAddress', remoteAddress)
      // => 192.168.0.23
      // => 192.168.122.1
      ...
      if (a[0] === b[0]) {
        ...
        if (a[1] === b[1]) {
          ...
          // Matches 192.168.122.*
          console.log('early-returning from holepunching because of match on first 3 triplets:', remoteAddress)
  ...
  console.log('Holepuncher returning best match', best.addr)
  return best.addr
}
