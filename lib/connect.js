...
async function holepunch (c, opts) {
  console.log('\n\n\nholepunching\n\n\n', 'connect:', c.connect, '\npayload holepunch:', c.connect.payload.holepunch, 'payload addresses4:', c.connect.payload.addresses4)
  ...
  if (payload.firewall === FIREWALL.OPEN || (relayed && !remoteHolepunchable)) {
    console.log('\n\nCase open firewall or not remote holepunchable')
  ...
  if (c.lan && relayed && clientAddress.host === serverAddress.host) {
    ...
    if (serverAddresses.length > 0) {
      ...
      console.log('my address:', myAddresses, 'matches addr:', addr)
      if (addr.host.startsWith('172.17')) {
        console.log('probably docker address-skipping over LAN attempt')
        isProbableDocker = true
      }

      // if (isProbableDocker) addr.host = '192.168.0.183'
  ...
  console.log('remote holepunchable?', remoteHolepunchable)
  if (!remoteHolepunchable) {
    maybeDestroyEncryptedSocket(c, CANNOT_HOLEPUNCH())
    return
  }

  console.log('\n\ndoing actual holepunching')
  ...
  console.log('diff address case while holepunching')
  ...
  console.log('checking early return with TODO on lan')
...
async function probeRound (c, serverAddress, serverRelay, retry) {
  ...
  console.log('starting probe round--opening puncher session to', serverAddress)
  ...
  console.log('holepunch reply:', reply)
